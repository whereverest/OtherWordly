    ...
    <svelte:fragment slot="content">
      ...
      <div
        class={`words-recap
                    ...
                    ${
                      !isPageLoaded &&
                      "level-results-page__words-recap--invisible"
                    }`}
      >
        <div class="words-recap__title">Words recap</div>

        <div
          class={`words-recap__words-list-n-description
                      ${
                        $isPortraitMode &&
                        "words-recap__words-list-n-description--vertical"
                      }`}
        >
          <!-- Words list -->
          <div
                ...
                {#each recapTestData as wordRecap (wordRecap.term)}
                  <li
                    class={`words-list__word
                              ${
                                wordRecap.term === selectedWord &&
                                "words-list__word--active"
                              }`}
          ...
          <div
            class={`description
                        ...
                        ${
                          $isPortraitMode &&
                          "words-recap__description--vertical"
                        }`}
          >
            {#if !selectedWord}
              <div class="description__title">Check out how you did!</div>
              <div class="description__hint">
                Tap <em>words</em> you hit
                <em>{$isPortraitMode ? "above" : "on the left"}</em><br />
                to see hints.
              </div>
            {:else}
              <div
                ...
                bind:innerHTML={recapTestData[
                  recapTestData.findIndex((item) => item.term === selectedWord)
                ].example}
              />
              <div
  ...
  .words-recap {
    ...
    &__words-list-n-description {
      ...
      &::before {
        ...
        max-height: 45vh;
        position: absolute;
        inset: calc(50% - 22.5vh) auto auto 25rem;
        overflow: hidden;
      ...
      &--vertical {
        ...
        &::before {
          ...
          min-height: 0.2rem;
          max-height: 0.2rem;
          inset: calc(40% + 2rem) auto auto 0;
          background-image: linear-gradient(
            to right,
            rgba(222, 222, 222, 0),
            rgba(222, 222, 222, 0.25),
