<script>
  ...
  // Stores
  import { isICloudSync, isAnalytics } from "@stores/options.js";
  ...
  $: {
    if ($isICloudSync !== undefined) {
      dispatchIOSEventWrapper({
        tapped: "OWJSMsgIsICloudSyncChange",
        value: $isICloudSync,
      });
    }
  }
  $: {
    if ($isAnalytics !== undefined) {
      dispatchIOSEventWrapper({
        tapped: "OWJSMsgIsAnalyticsChange",
        value: $isAnalytics,
      });
    }
  }
...
<div
  class={`privacy-n-data
              ${mixClass}`}
>
  <!-- Clear data modal -->
  <div
    class={`clear-data-modal
  ...
  >
    ...
    <div class="clear-data-modal__description">
      You are about to clear your data. <em
        >This will erase your entire history and reset OtherWordly.</em
      >
      You will lose all of your game progress.
      <span
        >It will also reset your iCloud Sync and your game data will be
        unrecoverable.</span
      >
  ...
  <ScrollableBlock
    class={`privacy-n-data__scrollable-block
                            ${
                              isClearDataModalVisible &&
                              "privacy-n-data__scrollable-block--invisible"
                            }`}
  >
    <div class="privacy-n-data__in-wrap">
      ...
      <p style="margin-bottom: 3.5rem;">
        Control what data is saved, what’s sent to iCloud, and what’s shared
        with us.
      </p>

      <div class="privacy-n-data__divider-horizontal" />
      ...
      <div
        class={`separate-section
                  ...
                  ${
                    $isPortraitMode &&
                    "privacy-n-data__separate-section--portrait-mode"
                  }
                  `}
      >
        <div
        ...
        <div
          class={`separate-section__col-right
                    ${
                      $isPortraitMode &&
                      "separate-section__col-right--portrait-mode"
                    }
                  `}
        >
          <p style="margin-bottom: 1.5rem;">
            Sync your game level progress and stars to your other iCloud
            devices.
          </p>

          {#if $isICloudSync}
            <p>
              <b> iCloud Sync is active. </b>
              Your level progress is automatically saved to your iCloud account and
              you can play OtherWordly across multiple devices. Your crystals are
              device-specific, and aren’t saved to iCloud.
            </p>
          {:else}
            <p>
              <b> iCloud Sync is inactive </b>
              and your level progress is not saved to the cloud. Turn on iCloud Sync
              if you want to play OtherWordly on other devices connected to your
              iCloud account.
            </p>
          {/if}
        </div>
      ...
      >
        ...
        >
          <p>
            Your game progress and cumulative stats are stored on your device.
            These stats are shown on the "Info" panel in "All-Time Stats." The
            number of stars you earned on each level appears on the main menu.
            You can clear your history and start over by resetting the game.
            (Once you clear your data, it can’t be retrieved.)
          </p>
        </div>
      </div>
      <!-- / Separate section -->
      ...
      >
        ...
        >
          {#if $isAnalytics}
            <p>
              <b> Analytics are active. </b>
              To improve OtherWordly, we collect basic information about how you
              play (your stats in levels), your device state, and any crashes you
              experience. This information is transmitted anonymously.
            </p>
          {:else}
            <p>
              <b> Analytics are not active. </b>
